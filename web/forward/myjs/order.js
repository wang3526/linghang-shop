...
function addEle(data,tmp1,tmp) {
    for (var i in data){
        var mode=data[i];
        var createTime=mode.createTime;
        var date=new Date(createTime);
        var time=formatDate(date);
        $("#order-list"+tmp).append('<div className="order-status1">'+
            '<div className="order-title">'+
                '<div className="dd-num">订单编号：<a id="order-code'+i+'" href="javascript:;">'+mode.code+'</a>'+
            ...
            '<div id="order-content'+tmp1+i+'" className="order-content">'+
                ...
                '<div id="order-right'+tmp1+i+'" className="order-right">'+
                    '<li id="td-amount'+tmp1+i+'" className="td td-amount">'+
                        '<div id="item-amount'+tmp1+i+'" className="item-amount">合计：'+Number(mode.payment)+'&nbsp;元'+
                            '<p>含运费：<span>'+mode.postFee+'元</span></p>'+
                        '</div>'+
                    '</li>'+
        ...
        for(var j in mode.list){
            ...
            $("#pro"+tmp1+i).append('<ul id="item-list" className="item-list">'+
                '<li id="td-item'+tmp1+i+j+'" className="td td-item">'+
                    '<div id="item-pic'+tmp1+i+j+'" className="item-pic">'+
                        '<a id="J_MakePoint'+tmp1+i+j+'" href="#" className="J_MakePoint">'+
                            '<img id="J_ItemImg'+tmp1+i+j+'" src="'+path+item.picPath+'" className="itempic J_ItemImg">'+
                        '</a>'+
                    '</div>'+
                ...
                '<li id="td-price'+tmp1+i+j+'" className="td td-price">'+
                    '<div id="item-price'+tmp1+i+j+'" className="item-price" style="margin-top: 18px">'+item.price+'/元</div>'+
                '</li>'+
                '<li id="td-number'+tmp1+i+j+'" className="td td-number">'+
                    '<div id="item-number'+tmp1+i+j+'" className="item-number" style="margin-top: 18px">'+
                        '<span>×</span>'+item.num+
                    '</div>'+
                '</li>'+
            ...
            if(mode.status==1){
                ...
                $("#item-operation"+tmp1+i+j).append('<a style="color: #000" onclick="drawback('+mode.code+')">退款</a>');
            }else if(mode.status==3){
                $("#item-operation"+tmp1+i+j).append('<a style="color: #000" onclick="drawback('+mode.code+')">退款/退货</a>');
        ...
        if(mode.status==1){
            $("#item-status"+tmp1+i).append('<p id="Mystatus-l1'+tmp1+i+'" className="Mystatus">等待买家付款</p>'+
                '<p id="order-info-l1'+tmp1+i+'" className="order-info"><a style="color: #000" onclick="closeOrder('+mode.code+')">取消订单</a></p>');
            $("#Mystatus-l1"+tmp1+i).addClass("Mystatus");
            $("#order-info-l1"+tmp1+i).addClass("order-info");
            $("#td-change"+tmp1+i).append('<div id="am-btn-danger-l1'+tmp1+i+'" className="am-btn am-btn-danger anniu"><a style="color:#fff" href="../home/payway.html?code='+mode.code+'">一键付款</a></div>');
            ...
            $("#item-status"+tmp1+i).append('<p id="Mystatus-l2'+tmp1+i+'" className="Mystatus">买家已付款</p>'+
                '<p id="order-info-l2'+tmp1+i+'" className="order-info"><a href="orderinfo.html?code='+mode.code+'">订单详情</a></p>');
            ...
            $("#item-status"+tmp1+i).append('<p id="Mystatus-l3'+tmp1+i+'" className="Mystatus">卖家已发货</p>'+
                '<p id="order-info-l3-1'+tmp1+i+'" className="order-info"><a href="orderinfo.html?code='+mode.code+'">订单详情</a></p>'+
            ...
            $("#td-change"+tmp1+i).append('<div id="am-btn-danger-l3'+tmp1+i+'" className="am-btn am-btn-danger anniu"><a style="color:#fff" onclick="receiptStatus('+mode.code+')">确认收货</a></div>');
            ...
            $("#item-status"+tmp1+i).append('<p id="Mystatus-l4'+tmp1+i+'" className="Mystatus">交易成功</p>'+
                '<p id="order-info-l4-1'+tmp1+i+'" className="order-info"><a href="orderinfo.html?code='+mode.code+'">订单详情</a></p>'+
            ...
            $("#td-change"+tmp1+i).append('<div id="am-btn-danger-l4'+tmp1+i+'" className="am-btn am-btn-danger anniu"><a style="color: #fff" href="commentlist.html?code='+mode.code+'" onclick="closeOrder('+mode.code+')" >评价商品</a></div>');
            ...
            $("#item-status"+tmp1+i).append('<p id="Mystatus-l5'+tmp1+i+'" className="Mystatus">退款中</p>'+
                '<p id="order-info-l5-1'+tmp1+i+'" className="order-info"><a href="orderinfo.html?code='+mode.code+'">订单详情</a></p>');
