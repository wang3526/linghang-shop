...
function addEle(data) {
    ...
    $("#order-name").text(data.shippingName);
    $(".package-number").text(data.shippingCode);
    $.get(path+"/address/findById.do",{"id":data.raId},function (address) {
            $(".new-txt").text(address.name);
            $(".new-txt-rd2").text(address.phone);
            $(".province").text(address.province);
            $(".city").text(address.city);
            $(".dist").text(address.county);
    ...
    $("#order-main").append('<div className="order-status1">'+
        '<div className="order-title">'+
            '<div className="dd-num">订单编号：<a href="javascript:;">'+data.code+'</a>'+
        ...
        '<div id="order-content" className="order-content">'+
            ...
            '<div id="order-right" className="order-right">'+
                '<li id="td-amount" className="td td-amount">'+
                    '<div id="item-amount" className="item-amount">合计：'+(Number(data.payment)+Number(data.postFee))+'&nbsp;元'+
                        '<p>含运费：<span>'+data.postFee+'元</span></p>'+
                    '</div>'+
                '</li>'+
    ...
    for(var j in data.list){
        ...
        $("#pro").append('<ul id="item-list" className="item-list">'+
            '<li id="td-item'+j+'" className="td td-item">'+
                '<div id="item-pic'+j+'" className="item-pic">'+
                    '<a id="J_MakePoint'+j+'" href="#" className="J_MakePoint">'+
                        '<img id="J_ItemImg'+j+'" src="'+path+item.picPath+'" className="itempic J_ItemImg">'+
                    '</a>'+
                '</div>'+
            ...
            '<li id="td-price'+j+'" className="td td-price">'+
                '<div id="item-price'+j+'" className="item-price" style="margin-top: 18px">'+item.price+'&nbsp;元</div>'+
            '</li>'+
            '<li id="td-number'+j+'" className="td td-number">'+
                '<div id="item-number'+j+'" className="item-number" style="margin-top: 18px">'+
                    '<span>×</span>'+item.num+
                '</div>'+
            '</li>'+
            '<li id="td-operation'+j+'" className="td td-operation">'+
                '<div id="item-operation'+j+'" className="item-operation" style="margin-top: 20px">'+
            '</div>'+
            '</li>'+
            '</ul>');
       ...
       if(data.status==2){
            $("#item-operation"+j).append('<a style="color: #000" onclick="drawback('+data.code+')">退款</a>');
        }else if(data.status==3){
            $("#item-operation"+j).append('<a style="color: #000" onclick="drawback('+data.code+')">退款/退货</a>');
        }else if(data.status==4){
            $("#item-operation"+j).append('<a style="color: #000" onclick="drawback('+data.code+')">退款/退货</a>');
        }
    }
    if(data.status==2){
        ...
        $("#td-change").append('<div id="am-btn-danger-l3" className="am-btn am-btn-danger anniu"><a style="color: #fff" onclick="receiptStatus('+data.code+')">确认收货</a></div>');
        ...
        $("#td-change").append('<div id="am-btn-danger-l4" className="am-btn am-btn-danger anniu"><a style="color: #fff" onclick="closeOrder('+data.code+')" href="commentlist.html?code='+data.code+'">评价商品</a></div>');
